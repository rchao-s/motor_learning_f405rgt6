...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
	if (huart->Instance == USART3)
    {
        ...
        {		
            ...
            if (duty_target < 40) duty_target = 40;
            if (duty_target > 99) duty_target = 99;

            //pwm_ccr = duty;

            dma_printf("pwm_target: %d\r\n", duty_target);
					
        }
        else if (strcmp((char*)RxBuff, "start") == 0)
        {
            START_CMD = 1;
            dma_printf("START_CMD = 1\r\n");
        }
        else if (strcmp((char*)RxBuff, "stop") == 0)
        {
            START_CMD = 0;
            dma_printf("START_CMD = 0\r\n");
        }
				 else if (strcmp((char*)RxBuff, "status") == 0)
        {
            
            dma_printf("START_CMD = %d,Duty = %d\r\n",START_CMD, pwm_ccr);
        }
        else
        {
            dma_printf("Unknown cmd: %s, Size=%d\r\n", RxBuff, Size);
        }
...
void dma_printf(const char *format, ...)
{
    static char buffer[256];  // 静态缓冲区，避免被销毁
    va_list args;
